...
int getStorageStatus(STORAGE_INFO_T *st)
{
	...
	if(nvram_get_int("enable_webdav")) {	
		...
		if(nvram_get_int("st_webdav_mode")) {			
			st->u.wt.HttpType = EXTEND_WEBDAV_TYPE_HTTPS;
			st->u.wt.Port = htons(nvram_get_int("webdav_https_port"));
		}	
		else {
			st->u.wt.HttpType = EXTEND_WEBDAV_TYPE_HTTP;
			st->u.wt.Port = htons(nvram_get_int("webdav_http_port"));
		}
	} 
	else {	
