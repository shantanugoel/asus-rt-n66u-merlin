...
usb_dbg("kill pppd.\n");
	...
	if(isSerialNode(device_name)){
		// Set the default node. Because there is no Int endpoint in some dongles.
		...
		if(!strcmp(current_value, "")){
			...
			else if(cur_val != -1 && cur_val >= tmp_val){ // get the bigger node.
...
usb_dbg("(%s): cur_val=%d, tmp_val=%d.\n", device_name, cur_val, tmp_val);
	...
	if(!nvram_match("wans_mode", "off")){
		...
		if(wan_unit != WAN_UNIT_MAX){
usb_dbg("got tty nodes and notify restart wan(%d)...\n", wan_unit);
			memset(cmd, 0, 32);
