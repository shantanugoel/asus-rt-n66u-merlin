...
{
	// DSLTODO
	// HOW TO INTEGRAITE USB WAN
	get_dualwan_values_once();
	if (m_secondary_if == WANS_DUALWAN_IF_NONE)
	{
		if (m_primary_if == WANS_DUALWAN_IF_DSL)
		{
			if (nvram_match("dsltmp_adslsyncsts","up")) return 1;
			return 0;
		}
		if (m_primary_if == WANS_DUALWAN_IF_LAN)
		{
