...
var WEBDAV_status = get_webdav_status(); // webdav

var AM_to_cifs = get_share_management_status("cifs");  // Account Management for Network-Neighborhood
var AM_to_ftp = get_share_management_status("ftp");  // Account Management for FTP
var AM_to_webdav = get_share_management_status("webdav");  // Account Management for webdav
...
function switchAppStatus(protocol){  // turn on/off the share
	...
	if(protocol == "cifs"){
		...
		status = this.WEBDAV_status;
		
		confirm_str_off = "Do you want to disable it?"; // tmp test
		confirm_str_on = "Do you want to enable it?"; // tmp test
	}
...
function showShareStatusControl(protocol){
	...
	if(protocol == "cifs"){
		...
		str_on = "<#btn_Enable#>"; // tmp test
		str_off = "<#btn_disable#>"; // tmp test
	}
	else
		return;
...
function setSelectAccount(selectedObj){
	...
	for(i=0; i<accounts.length; i++){
		if(accounts[i] == this.selectedAccount)
			_index = i;
	}
	$("_acc_webdavproxy").checked = parseInt(acc_webdavproxy[_index]);
	...
	for(var i = 0; i < pool_devices().length; ++i){
		...
		for(var j = 1; j < permissions.length; ++j){
			...
			if(protocol == "cifs")
				...
				showPermissionRadio(folderBarCode, permissions[j][3]);
			else{
...
function get_permission_of_folder(accountName, poolName, folderName, protocol){
	...
	for(var i = 1; i < permissions.length; ++i) // sync 02.07
...
function change_acc_webdavproxy(_value){
	$("changePermissionBtn").className = "button_gen";
	$("changePermissionBtn").disabled = false;

	if(_index < acc_webdavproxy.length)
		acc_webdavproxy[_index] = (_value == "on") ? 1 : 0;
	else 
		alert("!###Account error!");
	...
	showLoading();
	document.form.acc_webdavproxy.value = $("acc_webdavproxy").value;
...
<input type="hidden" name="acc_webdavproxy" id="acc_webdavproxy" value="<% nvram_get("acc_webdavproxy"); %>">
</form>
...
<input type="hidden" name="acc_webdavproxy" value="<% nvram_get("acc_webdavproxy"); %>">
...
<tr>
	<td bgcolor="#4D595D">
		  <div>&nbsp;</div>
			<div style="width:730px">
				<table width="730px">
					<tr>
						<td align="left">
							<span class="formfonttitle"><#menu5_4#> - !###WebDav</span>
						</td>
						<td align="right">
		  ...
		  <div class="formfontdesc">!###Set the account and permission of WebDav service.</div>
			...
			<!-- The table of share.   start-------------------------------------------------------------------------------->
			<div id="shareStatus" style="height:auto;">
			  		...
			  		<table width="480"  border="0" cellspacing="0" cellpadding="0" class="FileStatusTitle">
		  	    		<tr>
		    	  			<td width="290" height="20" align="left">
									<div><input type="checkbox" id="_acc_webdavproxy" class="input" onChange="change_acc_webdavproxy(this.value);">!###Webdav Proxy for this user.</div>
				    			<div class="machineName" style="margin-top:5px;"><% nvram_get("productid"); %></div>
				    			<!--<img src="/images/500icon.gif" width="20" height="20" hspace="2" align="absmiddle">-->
				    		</td>
				  		<td>
				    			<br/><div id="permissionTitle" style="margin-top:5px;"></div>
				  		</td>
		    			</tr>
			  		</table>
			 	 <!-- the tree of folders -->
